...
const regexMatchAll = (input, regex) => {
  const matches = [];
  let match;
...
function part1(input) {
  ...
  matches.forEach((match, index) => {
    ...
    //   match.index,
    //   input[match.index - 1],
    //   input[match.index + match.match.length],
    //   (match.index - (cols + 3), match.index - (cols + 3) + match.match.length + 2),
    //   (match.index + (cols + 1), match.index + (cols + 3) + match.match.length),
    // );
    if (input[match.index - 1] !== '.' || input[match.index + match.match.length] !== '.'
      || /[^\.]/.test(input.substring(match.index - (cols + 3), match.index - (cols + 3) + match.match.length + 2))
      || /[^\.]/.test(input.substring(match.index + (cols + 1), match.index + (cols + 3) + match.match.length))
    ) {
      realMatches.push(match);
    }
  });

  return realMatches.reduce((acc, match) => {
    return acc + parseInt(match.match);
  }, 0);
}
...
function part2(input) {
  ...
  matches.forEach((match, index) => {
    let prev = parseInt(
      (
        /[0-9]/.test(input.substring(match.index - 1, match.index))
          ? (/[0-9]+$/.exec(input.substring(match.index - 5, match.index)) ?? [1])
          : [1]
      )[0]
    );
    let next = parseInt(
      (
        /[0-9]/.test(input.substring(match.index + match.match.length, match.index + match.match.length + 1))
          ? /[0-9]+/.exec(input.substring(match.index + match.match.length, match.index + match.match.length + 5)) ?? [1]
          : [1]
      )[0]
    );
    let before = input.substring(match.index - (cols + 5), match.index - (cols + 3) + match.match.length + 4);
    before = regexMatchAll(before, /[0-9]+/g).filter((match) => match.index + match.match.length <= 2 || match.index >= 5 ? false : true).map((match) => parseInt(match.match)) || 1;
    let after = input.substring(match.index + (cols - 1), match.index + (cols + 3) + match.match.length + 2);
    after = regexMatchAll(after, /[0-9]+/g).filter((match) => match.index + match.match.length <= 2 || match.index >= 5 ? false : true).map((match) => parseInt(match.match)) || 1;

    const contacts = [prev, next, ...before, ...after].filter((match) => match > 1);

    console.log(contacts);

    if (contacts.length === 2) {
      realMatches.push(contacts.reduce((acc, match) => acc * match, 1));
