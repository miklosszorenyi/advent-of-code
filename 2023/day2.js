...
function part1() {
  ...
  games.forEach((game) => {
    const parts = game.replace('Game ', '').split(/[:;]/).map((match) => match.trim());
    const gameNr = parseInt(parts.splice(0, 1)[0]);
    const sets = parts.map((set) => set.split(',').map((match) => match.trim())).map((set) => set.reduce((acc, set) => {
      const match = set.split(' ');
      return {
        ...acc,
        [match[1]]: parseInt(match[0])
      }
    }, {}));
    ...
    for (let i in available) {
      sets.forEach((set) => {
        // console.log(set[i], available[i]);
        if ((set[i] ?? 0) > available[i]) {
          valid = false;
        }
      });
    }

    validGames += valid ? gameNr : 0;
  });
...
function part2() {
  ...
  games.forEach((game) => {
    const parts = game.replace('Game ', '').split(/[:;]/).map((match) => match.trim());
    score += Object.values(parts.map((set) => set.split(',').map((match) => match.trim())).map((set) => set.reduce((acc, set) => {
      const match = set.split(' ');
      return {
        ...acc,
        [match[1]]: parseInt(match[0])
      ...
      return {
        ...acc,
        ...{
          red: !acc.red ? set.red : (set.red > acc.red ? set.red : acc.red),
          green: !acc.green ? set.green : (set.green > acc.green ? set.green : acc.green),
          blue: !acc.blue ? set.blue : (set.blue > acc.blue ? set.blue : acc.blue),
        }
      }
    }, {})).reduce((acc, set) => {
      return acc * set;
