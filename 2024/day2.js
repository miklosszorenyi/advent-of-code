...
function isCorrect(row) {
  let correct = true;
  const rowCopy = row.map(num => num);
  rowCopy.forEach((num, i) => {
    const diff = Math.abs(num - rowCopy[i - 1]);
    if (i && (diff < 1 || diff > 3)) correct = false;
  });

  if (rowCopy[0] < rowCopy[1] && rowCopy.join('') != rowCopy.sort((a, b) => a - b).join('')) correct = false;
  if (rowCopy[0] > rowCopy[1] && rowCopy.join('') != rowCopy.sort((a, b) => b - a).join('')) correct = false;
...
function part1(input) {
  return input.map(row => {
    return isCorrect(row) ? 1 : 0;
  }).reduce((a, b) => a + b);
}

function part2(input) {
  return input.map(row => {
    let correct = isCorrect(row);
    if (!correct) {
      for (let i in row) {
        let rowCopy = row.map(num => num);
        rowCopy.splice(i, 1);
        correct = isCorrect(rowCopy);
        if (correct) break;
      }
    }
    return correct ? 1 : 0;
