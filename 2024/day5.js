...
function get_correct_rows(input1, input2) {
  const correct_rows = [];
  const incorrect_rows = [];
  input2.forEach((row) => {
    const correct_numbers = {};
    row.forEach((nr1, i1) => {
      if (correct_numbers[nr1] === undefined) {
        correct_numbers[nr1] = true;
      }
      row.forEach((nr2, i2) => {
        if ((i1 < i2 && input1.includes(`${nr1}|${nr2}`)) && correct_numbers[nr1] !== false) {
          correct_numbers[nr1] = true;
        } else if (i1 > i2 && input1.includes(`${nr1}|${nr2}`)) {
          correct_numbers[nr1] = false;
        }
      })
    });
    if (Object.values(correct_numbers).filter(v => v).length == row.length) {
      correct_rows.push(row)
    } else {
...
function part1(input1, input2) {
  const correct_rows = get_correct_rows(input1, input2)[0];
  return correct_rows.map(row => row[Math.round((row.length - 1) / 2)]).reduce((a, b) => a + b, 0);
}

function part2(input1, input2) {
  const incorrect_rows = get_correct_rows(input1, input2)[1];
  const correct_rows = incorrect_rows.map(row => row.map(nr => nr).sort((a, b) => input1.includes(`${a}|${b}`) ? -1 : 1));

  return correct_rows.map(row => row[Math.round((row.length - 1) / 2)]).reduce((a, b) => a + b, 0);
}

console.log(part2(final_input_1, final_input_2));