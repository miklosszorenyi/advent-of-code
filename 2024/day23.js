const { uniq, flatMap, intersection } = require("lodash");
const { generatePermutations, collectGraph, DFS, bronKerbosch } = require("./libs");

const test_input = [
  'kh-tc', 'qp-kh', 'de-cg', 'ka-co', 'yn-aq', 'qp-ub', 'cg-tb', 'vc-aq', 'tb-ka', 'wh-tc', 'yn-cg',
  'kh-ub', 'ta-co', 'de-co', 'tc-td', 'tb-wq', 'wh-td', 'ta-ka', 'td-qp', 'aq-cg', 'wq-ub', 'ub-vc',
  'de-ta', 'wq-aq', 'wq-vc', 'wh-yn', 'ka-de', 'kh-ta', 'co-tc', 'wh-qp', 'tb-vc', 'td-yn',
].map((str) => str.split('-'));

const final_input = [
  'ur-jg',
  'sh-sz',
  'nv-hr',
  'bm-xn',
  'np-xc',
  'ii-hj',
  'jf-zv',
  'bz-sn',
  'iw-fl',
  'tw-vd',
  'yk-xc',
  'hz-uv',
  'hh-fy',
  'ng-mx',
  'ps-rz',
  'jx-mk',
  'sf-gb',
  'qz-ec',
  'hw-du',
  'if-nk',
  'mb-ob',
  'ln-fc',
  'sc-rr',
  'ss-rn',
  'hn-fx',
  'aw-qu',
  'km-mj',
  'uk-ho',
  'tu-ah',
  'me-ld',
  'bi-lg',
  'xe-zm',
  'cs-rn',
  'tj-tf',
  'qk-yi',
  'bm-dl',
  'fa-im',
  'of-bk',
  'ib-jf',
  'sa-lb',
  'ws-fg',
  'ev-yt',
  'qb-aq',
  'qt-aa',
  'gu-su',
  'si-zi',
  'kf-ix',
  'as-lx',
  'ss-fe',
  'il-ea',
  'sv-la',
  'nf-mg',
  'un-ov',
  'lq-jx',
  'vm-tc',
  'ac-gd',
  'oc-ec',
  'aj-ia',
  'bg-ul',
  'ok-xf',
  'yj-tc',
  'js-la',
  'nf-xe',
  'gb-gi',
  'vq-eg',
  'iy-ka',
  'xd-di',
  'gq-oo',
  'wz-hx',
  'fu-ij',
  'gx-ob',
  'tk-ea',
  'rx-ea',
  'lt-wm',
  'yy-me',
  're-jy',
  'wb-sl',
  'de-lg',
  'eh-jg',
  'kj-jk',
  'xp-ph',
  'lb-pf',
  'oc-si',
  'cv-yg',
  'hi-lk',
  'fm-jw',
  'ub-ey',
  'ir-de',
  'yj-ek',
  'pb-gm',
  'as-xq',
  'ok-kt',
  'rd-md',
  'aw-gv',
  'wn-qn',
  'js-xq',
  'fb-rx',
  'ix-jt',
  'ze-gi',
  'um-cs',
  'hs-sa',
  'dt-ib',
  'ld-qe',
  'zj-cf',
  'li-sn',
  'dw-nr',
  'fv-ly',
  'zr-qb',
  'ge-wj',
  'mw-cj',
  'lw-pu',
  'et-pc',
  'ze-sf',
  'ky-km',
  'wi-oq',
  'en-qi',
  'qm-ym',
  'rr-mx',
  've-me',
  'le-ao',
  'lz-bc',
  'xw-ul',
  'ib-zc',
  'dy-mx',
  'yy-qe',
  'ka-nr',
  'me-gq',
  'ga-lh',
  'al-bi',
  'cj-gs',
  'ga-mv',
  'uf-yx',
  'aw-og',
  'nl-ys',
  'fk-gv',
  'ym-ti',
  'oh-sy',
  'pt-tb',
  'ze-cp',
  'fa-hz',
  'iq-cl',
  'sv-lc',
  'xc-ec',
  'nj-kb',
  'kj-mk',
  'fu-qe',
  'll-bb',
  'zt-ra',
  'mh-sh',
  'ei-gg',
  'tb-oa',
  'ic-fo',
  'lh-ll',
  'rp-qi',
  'yi-kw',
  'qm-yo',
  'zm-nc',
  'gs-fv',
  'iz-zc',
  'oo-qe',
  'zr-zj',
  'cg-hr',
  'nd-qu',
  'fc-aq',
  'zm-oy',
  'pw-zp',
  'ya-ws',
  'fl-bq',
  'la-kd',
  'vh-cu',
  'tq-wz',
  'xm-ef',
  'nu-fc',
  'wu-zj',
  'cj-fv',
  'xp-ak',
  'ch-oc',
  'ez-nz',
  'eg-qj',
  'ak-nl',
  'au-ak',
  'lb-hs',
  'lx-sb',
  'xe-ac',
  'dk-vs',
  'nh-fj',
  'vh-ug',
  'sf-xf',
  'jp-po',
  'iw-kv',
  'wu-aq',
  'lo-ii',
  'le-yy',
  'ua-ap',
  'nd-ta',
  'xd-dm',
  'nl-el',
  'tq-nz',
  'gl-gx',
  'dc-mu',
  'yc-bq',
  'oe-xf',
  'nv-kk',
  'kj-tx',
  'wm-oi',
  'yq-ug',
  'no-xq',
  'qe-ao',
  'bz-se',
  'ou-sx',
  'lh-mv',
  'uc-yh',
  'wx-pw',
  'ev-sp',
  'nd-rl',
  'xw-kx',
  'oa-yd',
  'gp-qs',
  'pq-wl',
  'ss-qo',
  'jy-lk',
  'gh-bs',
  'nv-ei',
  'xs-un',
  'fr-rp',
  'rj-ug',
  'rr-bf',
  'rl-qt',
  'pz-qt',
  'nc-mg',
  'cn-qo',
  'li-se',
  'uc-rz',
  'ps-yh',
  'wc-wd',
  'md-ce',
  'sc-ce',
  'zc-jf',
  'te-vo',
  'kx-we',
  'gb-cp',
  'cm-wv',
  'lf-jn',
  'yh-wb',
  'rr-bv',
  'uv-fa',
  'tj-kf',
  'vh-bc',
  'xp-ku',
  'ge-of',
  'rz-vi',
  'mj-tt',
  'rx-mv',
  'qy-bv',
  'ye-ap',
  'ym-sd',
  'ka-wv',
  'qt-ta',
  'hw-yr',
  'rk-hk',
  'jp-gn',
  'yr-cm',
  'yg-au',
  'uv-gp',
  'ly-nk',
  'aa-ih',
  'xu-dw',
  'hm-ui',
  'pu-tg',
  'ry-yt',
  'ii-tq',
  'vp-xr',
  'ho-iw',
  'jy-zx',
  'lz-cd',
  'ty-iw',
  'ie-pb',
  'vt-uj',
  'ad-nw',
  'me-oo',
  'wv-nr',
  'mw-pz',
  'wf-cf',
  'ox-po',
  'ub-qs',
  'zb-pw',
  'xw-rk',
  'pd-zh',
  'sx-km',
  'wx-kq',
  'lb-jt',
  'tw-xt',
  'au-sx',
  'ta-wi',
  'pu-fx',
  'wn-qv',
  'tk-ka',
  'mb-so',
  'ui-oh',
  'ge-bk',
  'nz-xl',
  'se-rk',
  'nd-vg',
  'an-ns',
  'gj-zt',
  'tf-zh',
  'tk-tl',
  'ev-sv',
  'rm-sb',
  'cu-cy',
  'ef-rf',
  'tq-lo',
  'yn-ix',
  'wp-nk',
  'os-ck',
  'hw-oi',
  'xe-tu',
  'tn-pd',
  'nj-hs',
  'gi-ji',
  'aq-uz',
  'kg-mj',
  'ul-rc',
  'ns-hf',
  'qu-mw',
  'sq-tk',
  'rf-tt',
  'um-rf',
  'uf-lc',
  'ef-gs',
  'gc-pu',
  'dc-if',
  'wo-sl',
  'oo-nx',
  'rl-gr',
  'mv-er',
  'on-rc',
  'aq-fx',
  'ku-ak',
  'ox-wg',
  'kt-ze',
  'wy-si',
  'qe-wq',
  'kv-zt',
  'bi-ju',
  'dk-un',
  'yd-ot',
  'vd-ie',
  'eq-wz',
  'aq-nu',
  'vy-ws',
  'qs-ey',
  'aw-re',
  'sf-qq',
  'gi-ok',
  'wy-cq',
  'qh-ds',
  'al-vv',
  'hw-yv',
  'tl-sq',
  'dp-eh',
  'in-md',
  'xc-iv',
  'de-ma',
  'ic-vt',
  'kq-zb',
  'mu-tp',
  'xe-oy',
  'nd-mw',
  'io-eh',
  'yv-ia',
  'cw-kx',
  'nv-ua',
  'cd-kv',
  'ky-kg',
  'dq-gz',
  'zx-ju',
  'jc-xk',
  'hm-uk',
  'cl-vt',
  'ba-fz',
  'or-we',
  'df-sy',
  'ui-df',
  'hd-oz',
  'wq-dh',
  'ai-rj',
  'di-lt',
  'sh-mk',
  'nf-gd',
  'oa-ot',
  'xp-dk',
  'tl-ka',
  'sp-sv',
  'ln-jx',
  'xu-ka',
  'tg-gw',
  'mi-pa',
  'dc-wx',
  'cb-jk',
  'vq-hx',
  'sf-kt',
  'zx-ma',
  'du-cm',
  'ii-eg',
  'lz-ij',
  'ur-of',
  'by-zf',
  'ch-ub',
  'nc-ah',
  'lm-pj',
  'jd-zy',
  'tw-xy',
  'gv-po',
  'wc-oy',
  'gj-iw',
  'zc-dt',
  'bl-gw',
  'ez-xl',
  'pn-lr',
  'tc-wb',
  'eh-rm',
  'lf-di',
  'tw-fx',
  'yd-ya',
  'vs-ap',
  'sz-tx',
  'vx-vh',
  'no-bk',
  'xm-ue',
  'mp-yv',
  'as-sb',
  'rg-wv',
  'yq-hw',
  'av-kq',
  'wu-bu',
  'qm-fi',
  'mw-ta',
  'gg-yb',
  'fz-xz',
  'dd-zm',
  'oe-kt',
  'gr-qu',
  'il-tl',
  'dl-pa',
  'lq-cv',
  'sz-kj',
  'vb-oc',
  'qz-ek',
  'dj-ac',
  'gb-ca',
  'kj-mh',
  'wx-kb',
  'cn-pt',
  'us-jf',
  'rr-xp',
  'zt-ty',
  'em-xv',
  'ie-ro',
  'ys-el',
  'dc-ww',
  'rg-tk',
  'na-op',
  'er-ue',
  'pn-uk',
  'cd-ls',
  'zx-db',
  'rg-ig',
  'ar-yb',
  'cb-mk',
  'uq-bs',
  'qq-gi',
  'lr-uk',
  'gw-wz',
  'jf-so',
  'nw-jw',
  'fa-su',
  'ao-oo',
  'mg-gd',
  'xy-pu',
  'ac-dd',
  'ir-fw',
  'dv-hx',
  'lr-ui',
  'wf-ia',
  're-fk',
  'wl-zo',
  'pe-jh',
  'ch-xc',
  'do-ym',
  'aq-ln',
  'rz-ek',
  'um-rn',
  'pk-xn',
  'bs-zp',
  'np-vb',
  'io-zc',
  'nw-wp',
  'oa-ws',
  'yg-tt',
  'tk-ig',
  'oy-wd',
  'db-kz',
  'vi-yj',
  'ri-vx',
  'lq-km',
  'ej-hm',
  'dt-gl',
  'kd-as',
  'mg-zm',
  'is-pa',
  'ig-il',
  'pj-fk',
  'dk-ak',
  'dt-mb',
  'sp-ts',
  'yn-kf',
  'zf-ri',
  'lw-ia',
  'ho-cd',
  'cn-tb',
  'sv-te',
  'dx-tj',
  'vv-lg',
  'ka-rg',
  'fh-yx',
  'gr-qt',
  'oc-iv',
  'bl-hj',
  'op-mv',
  'uh-td',
  'yv-py',
  'ys-fv',
  'wz-qj',
  'wz-wt',
  'ys-ph',
  'zy-bd',
  'jz-qc',
  'rz-wb',
  'dj-gp',
  'fx-ki',
  'wl-kb',
  'qk-kw',
  'if-ly',
  'qc-ro',
  'pc-ov',
  'us-zc',
  'kt-qq',
  'en-ur',
  'ih-qu',
  'lt-dm',
  'ri-vh',
  'ib-zv',
  'mk-xr',
  'nu-wu',
  'pa-bd',
  'sb-la',
  'bs-wx',
  'nr-ig',
  'yk-cq',
  'et-el',
  'ii-hx',
  'zm-wc',
  'db-vv',
  'ec-cq',
  'sp-uf',
  'yi-wp',
  'ps-ek',
  'hi-jy',
  'uc-ps',
  'lq-mj',
  'dl-pk',
  'he-ds',
  'qq-cp',
  'op-rx',
  'jc-ue',
  'wt-ms',
  'uc-sl',
  'zf-vh',
  'ix-oz',
  'gz-xn',
  'qc-ie',
  'nz-wp',
  'ja-mo',
  'jg-os',
  'vx-px',
  'pj-lk',
  'go-uj',
  'zt-fu',
  'dl-jd',
  'ra-fl',
  'qs-fp',
  'en-mo',
  'qh-ym',
  'ga-ue',
  'om-ja',
  'kv-qs',
  'ap-hr',
  'ej-oh',
  'fr-os',
  'qh-xv',
  'is-th',
  'ku-et',
  'eg-gw',
  'ln-uz',
  'of-lr',
  'mq-nx',
  'ad-kp',
  'sn-se',
  'ck-fr',
  'hn-gc',
  'fk-kc',
  'oa-sc',
  'fv-fe',
  'sx-ee',
  'xq-ek',
  'er-ga',
  'fg-kr',
  'in-bf',
  'ng-bv',
  'vo-ts',
  'qk-ye',
  'et-ph',
  'oz-dx',
  'bl-dv',
  'fh-sp',
  'pn-hm',
  'nc-oy',
  'fm-xl',
  'ow-pf',
  'zf-cy',
  'kj-xr',
  'pb-uh',
  'ps-vm',
  'qm-do',
  'ob-or',
  'cw-rk',
  'ub-dj',
  'ov-vv',
  'wf-sn',
  'nt-yr',
  'hz-gu',
  'ov-ak',
  'xj-zv',
  'fn-or',
  'pc-ph',
  'ga-jc',
  'zu-uh',
  'wj-of',
  'xu-sq',
  'sl-ib',
  'bg-hk',
  'qy-ce',
  'pw-dc',
  'lb-kb',
  'ka-dw',
  'xw-or',
  'an-yr',
  'tn-oz',
  'lw-xt',
  'fe-rn',
  'jf-mb',
  'bs-dc',
  'kx-bg',
  'bd-jd',
  'hx-eq',
  'xm-er',
  'ao-wq',
  'jk-mh',
  'bk-df',
  'cx-dj',
  'bk-hm',
  'vh-ad',
  'zr-ln',
  'sd-ds',
  'ai-oi',
  'll-na',
  'fl-ij',
  'rd-rr',
  'ej-ge',
  'fo-fe',
  'uc-qz',
  'ur-ja',
  'uf-qv',
  'cd-ah',
  'mz-ck',
  'kg-lq',
  'xw-we',
  'ua-kk',
  'st-xt',
  'lm-ce',
  'ti-jl',
  'qy-sc',
  'ts-pn',
  'db-lg',
  'pz-ta',
  'uj-hh',
  'xn-lx',
  'wv-tl',
  'vq-eq',
  'ih-nd',
  'gr-aa',
  'kq-pw',
  'zi-ec',
  'ou-mj',
  'xt-tg',
  'gs-bp',
  'gr-oq',
  'sy-ej',
  'dy-rd',
  'pn-ui',
  'gw-hx',
  'do-qh',
  'lr-wj',
  'bq-ia',
  'su-xw',
  'ic-ba',
  'kx-xk',
  'nh-zr',
  'rc-kx',
  'nh-nu',
  'ib-iz',
  'he-lr',
  'bp-fo',
  'iw-ra',
  'me-qh',
  'jt-sa',
  'qy-lm',
  'uv-aj',
  'sd-ti',
  'ds-do',
  'kc-aw',
  'oz-zh',
  'ti-ii',
  'ju-de',
  'kw-ly',
  'rc-we',
  'dv-eq',
  'sv-yt',
  'jf-gl',
  'xs-pc',
  'ss-mm',
  'pt-kr',
  'gp-gu',
  'ip-sr',
  'ty-ra',
  'zv-gx',
  'nt-cm',
  'qo-vy',
  'ku-un',
  'pw-bs',
  'hp-fk',
  'vo-ev',
  'ue-bb',
  'et-nl',
  'wt-uh',
  'dh-oo',
  'th-vt',
  'kd-xq',
  'ye-gg',
  'np-oc',
  'uk-ej',
  'pc-el',
  'ix-zh',
  'lo-hx',
  'so-zv',
  'ja-gj',
  'ri-wn',
  'iq-wd',
  'zt-ho',
  'mw-oq',
  'bg-cw',
  'nk-nz',
  'ua-ar',
  'cu-oh',
  'qn-cu',
  'lx-js',
  'rd-sc',
  'dy-qy',
  'bl-qj',
  'kx-ul',
  'pz-ih',
  'tu-ac',
  'gu-ms',
  'qo-fg',
  'kk-gg',
  'tn-dx',
  'tj-hd',
  'xr-mh',
  'bd-bm',
  'cy-vh',
  'em-qm',
  'ja-tr',
  'ob-jf',
  'vy-kr',
  'dx-pd',
  'nj-qw',
  'hf-yq',
  'px-vh',
  'lw-xh',
  'gx-dt',
  'ow-jt',
  'tp-wx',
  'py-wf',
  'bz-wf',
  'gh-tp',
  'xh-st',
  'ez-kw',
  'jd-is',
  'lo-gw',
  'qi-mz',
  'nf-hm',
  'fz-fy',
  'hh-cl',
  'dp-la',
  'fe-mm',
  'iq-ic',
  'bb-mv',
  'dz-xz',
  'he-qh',
  'na-gd',
  'rf-cs',
  'et-ys',
  'kw-xl',
  'ge-hm',
  'kr-pf',
  'se-py',
  'ts-lc',
  'nk-ez',
  'ee-cv',
  'xe-mg',
  'tn-yu',
  'st-ki',
  'oi-an',
  'fe-cs',
  'ok-qq',
  'ga-na',
  'gm-af',
  'jp-xd',
  'xp-et',
  'lx-eh',
  'ma-bi',
  'gm-ms',
  'go-cl',
  'zp-wx',
  'af-jz',
  'ot-ws',
  'bg-or',
  'il-pm',
  'dj-ey',
  'gb-ji',
  'np-ec',
  'ge-ld',
  'nx-gq',
  'hj-tq',
  'ad-by',
  'ot-qo',
  'oy-tu',
  'nz-if',
  'sp-te',
  'ym-em',
  'tu-dd',
  'dh-le',
  'lf-wm',
  'sv-ry',
  'ig-ka',
  'tr-en',
  'sr-og',
  'he-ym',
  'ic-dz',
  'df-nj',
  'gu-qs',
  'tt-ky',
  'hs-jh',
  'xp-pc',
  'yx-ts',
  'ts-te',
  'wv-il',
  'ro-uh',
  'nf-zm',
  'fn-kx',
  'dt-xj',
  'kg-ap',
  'vi-yh',
  'hr-gg',
  'tw-lw',
  'gm-td',
  'nu-bu',
  'dx-zh',
  'au-lq',
  'pq-jh',
  'iv-ch',
  'kq-gh',
  'fa-qs',
  'um-fe',
  'tn-kf',
  'yd-tb',
  'em-jl',
  'of-df',
  'ia-bz',
  'cd-ra',
  'fa-gp',
  'ro-zu',
  'xn-dl',
  'fm-ly',
  'hr-kk',
  'cj-ss',
  'ww-gh',
  'tx-sh',
  'na-fb',
  'ot-vy',
  'jd-yh',
  'jt-kb',
  'gu-ub',
  'fu-fl',
  'wd-ah',
  'oo-xy',
  'wt-td',
  'cm-hf',
  'pf-pq',
  'zu-pb',
  'di-wg',
  'zu-af',
  'we-hk',
  'ki-vx',
  'jg-qi',
  'ms-jz',
  'dt-ob',
  'dh-te',
  'tl-ig',
  'zy-gz',
  'lg-av',
  'nw-kw',
  'bf-qy',
  'ef-rn',
  'yk-ch',
  'lb-nj',
  'rz-qz',
  'gm-vd',
  'rf-fe',
  'rg-nr',
  'kg-tt',
  'wo-qz',
  'jw-if',
  'jn-jp',
  'ou-ky',
  'fc-uz',
  'uv-qs',
  'nt-ns',
  'dl-zy',
  'io-no',
  'kf-hd',
  'sr-hp',
  'yo-do',
  'ee-ou',
  'fh-ev',
  'dz-go',
  'tu-wd',
  'me-qe',
  'tb-qo',
  'du-rj',
  'zy-nr',
  'tj-mf',
  'xr-dr',
  'na-lh',
  'el-un',
  'xv-do',
  'vg-qt',
  'cp-gi',
  'iv-vb',
  'qj-tq',
  'xj-gl',
  'yc-yv',
  'xz-uj',
  'ck-rp',
  'fj-qb',
  'bg-rk',
  'kb-qw',
  'wp-fm',
  'fe-ef',
  'xu-il',
  'dy-bv',
  'yx-te',
  'tt-ou',
  'ho-ra',
  'el-jw',
  'zf-kp',
  'jh-wl',
  'sx-ky',
  'oq-rl',
  'xn-bd',
  'ne-yd',
  'tc-uc',
  'tp-zb',
  'km-kz',
  'fa-ey',
  'so-ob',
  'dj-fa',
  'xr-rv',
  'yn-oz',
  'hl-ln',
  'mu-kq',
  'vs-uy',
  'xr-zv',
  'ao-dh',
  'qw-pf',
  'by-vx',
  'hl-nu',
  'tk-il',
  'ec-si',
  'yq-an',
  'ia-py',
  'cy-kp',
  'bu-uz',
  'vy-yd',
  'eh-la',
  'zc-mb',
  'hi-og',
  'lm-mx',
  'go-ic',
  'tw-pu',
  'bb-ga',
  'ra-lz',
  'he-sd',
  'cg-no',
  'gv-pj',
  'ox-wm',
  'yk-zn',
  'qo-ws',
  'ul-fn',
  'vq-ze',
  'cq-si',
  'vq-dv',
  'cf-ia',
  'kj-jx',
  'ma-ir',
  'nh-bu',
  'on-hk',
  'gl-ob',
  'zr-wu',
  'uq-zp',
  'oc-cq',
  'gz-gm',
  'zx-bi',
  'ss-cs',
  'em-ds',
  'ef-mm',
  'du-hf',
  'mq-me',
  'nl-un',
  'wi-ih',
  'zi-iv',
  'tw-tg',
  'tx-cb',
  'jn-dm',
  'bc-vx',
  'ga-ll',
  'ux-vt',
  'hk-xw',
  'mq-ao',
  'pq-ow',
  'bu-zr',
  'nw-ez',
  'wo-ps',
  'nj-pf',
  'yc-wf',
  'mi-th',
  'th-bd',
  'ck-mo',
  'gl-iz',
  'vx-cu',
  'ww-dy',
  'fr-tr',
  'us-zv',
  'en-kg',
  'rc-hk',
  'wz-bl',
  'ww-av',
  'vp-jk',
  'al-lg',
  'xt-fx',
  'im-gp',
  'sc-bf',
  'bi-db',
  'aj-py',
  'in-qy',
  'lf-um',
  'qj-eq',
  'qb-hl',
  'de-rn',
  'gv-re',
  'ls-ho',
  'rd-in',
  'zj-nh',
  'jz-bu',
  'tg-st',
  'no-kd',
  'yv-bz',
  'vm-zu',
  'gu-cx',
  'pd-hd',
  'og-ya',
  'dp-ok',
  'db-fw',
  'dw-sq',
  'tc-ek',
  'hs-pq',
  'qu-vg',
  'dy-sc',
  'nz-kw',
  'mj-ee',
  'jc-rx',
  'oz-tj',
  'cj-bp',
  'zp-dc',
  'bm-zy',
  'zu-jz',
  'fe-bp',
  'ra-ls',
  'kv-fl',
  'jw-yi',
  'yb-nv',
  'ky-lq',
  'mq-pq',
  'sx-pe',
  'gb-oe',
  'ti-do',
  'gn-po',
  'gn-dm',
  'yv-sn',
  'iy-lw',
  'hn-ki',
  'vo-sv',
  'eh-kd',
  'px-cy',
  'wi-vg',
  'sh-cb',
  'iq-uj',
  'op-bb',
  'fj-zr',
  'hl-aq',
  'jj-cq',
  'gg-vs',
  'zn-vo',
  'yg-kc',
  'bc-kp',
  'cg-lx',
  'kr-ya',
  'nw-fm',
  'hn-tg',
  'ls-fu',
  'nc-tu',
  'tu-gd',
  'ls-ij',
  'nd-oq',
  'ad-ri',
  'oy-mg',
  'bu-hl',
  'mo-rp',
  'of-sy',
  'nv-ye',
  'zb-dc',
  'aw-hp',
  'tb-fg',
  'up-ma',
  'oe-cp',
  'ly-xl',
  'sy-wj',
  'lh-rx',
  'gp-su',
  'js-io',
  'oq-pz',
  'ym-xv',
  'tn-ix',
  're-pj',
  'ub-fp',
  'kc-hi',
  'qz-vi',
  'li-bq',
  'dc-kq',
  'qk-fm',
  'gj-ra',
  'kf-yu',
  'mz-tr',
  'tf-ix',
  'tj-zh',
  'nd-pz',
  'bc-ri',
  'zp-ww',
  'xv-sd',
  'ys-ov',
  'mg-wc',
  'ui-uk',
  'fj-fc',
  'ry-uf',
  'ux-ic',
  'cl-fz',
  'oz-pd',
  'lb-zo',
  'og-hp',
  'iz-ob',
  'qe-dh',
  'qb-ln',
  'lh-xm',
  'ie-jz',
  'vs-ar',
  'hl-yo',
  'no-sb',
  'oc-wy',
  'bi-de',
  'ap-uy',
  'ne-pt',
  'tc-qz',
  'qe-ve',
  'ib-so',
  'wj-na',
  'ye-hr',
  'rg-pm',
  'md-bf',
  'ps-yj',
  'cb-kj',
  'sy-uk',
  'lm-bf',
  'ei-lk',
  'vo-uf',
  'kc-lk',
  'ju-kz',
  'ly-wp',
  'ck-jg',
  'xe-dd',
  'af-ms',
  'cm-ns',
  'bl-qn',
  'ti-ov',
  'zm-wd',
  'cy-wn',
  'ku-el',
  'go-fz',
  'xr-cb',
  'jw-kw',
  'go-vt',
  'xf-cp',
  'iv-np',
  'cl-uj',
  'ma-lg',
  'ea-nr',
  'uh-ms',
  'yh-qz',
  'xp-el',
  'yu-dx',
  'jn-xd',
  'zv-gl',
  'kt-gi',
  'oh-bk',
  'yc-mp',
  'gz-jd',
  'aq-fj',
  'dr-tx',
  'uc-uj',
  'vt-xz',
  'ys-un',
  'bp-cs',
  'fl-zt',
  'jd-bm',
  'kd-rm',
  'cn-vy',
  'tf-mf',
  'kj-rv',
  'tr-pk',
  'vb-zi',
  'ro-td',
  'nf-oy',
  'ce-rr',
  'wn-kp',
  'oz-tf',
  'lr-ge',
  'dq-dl',
  'fr-yy',
  'ev-qv',
  'wx-cn',
  'gd-oy',
  'pj-jy',
  'gc-tg',
  'hh-vt',
  'ok-gb',
  'kr-ws',
  'tp-dc',
  'jn-po',
  'ku-ph',
  'js-cg',
  'fy-ux',
  'zx-al',
  'sd-sp',
  'jj-gb',
  've-hf',
  'sr-hi',
  'km-yg',
  'mg-wn',
  'rl-ih',
  'yt-lc',
  'qs-im',
  'ai-yq',
  'hk-ul',
  'ui-ej',
  'iu-dm',
  'ok-ca',
  'sd-fi',
  'qc-ms',
  'th-jd',
  'mo-os',
  'nu-ai',
  'wv-tk',
  'wx-uq',
  'kr-qo',
  'xj-gx',
  'mh-dr',
  'ow-hs',
  'dz-hh',
  'oy-dd',
  'oq-aa',
  'ty-fl',
  'js-xe',
  'kf-tf',
  'af-pb',
  'lq-tt',
  'bu-aq',
  'wp-ez',
  'ti-fi',
  'lc-sp',
  'vp-mk',
  'vm-ek',
  'el-ov',
  'vd-qc',
  'lw-gc',
  'lt-yi',
  'em-he',
  'tc-sl',
  'fv-ef',
  'dw-tk',
  'nc-ac',
  'xm-na',
  'gc-xh',
  'pn-ge',
  'ca-jj',
  'pd-kf',
  're-lk',
  'yi-xl',
  'vi-uc',
  'xs-dk',
  'lx-dp',
  'si-ui',
  'vb-wy',
  'ch-si',
  'cg-dp',
  'cp-jj',
  'vt-iq',
  'ir-vv',
  'fj-hl',
  'oo-yy',
  'oa-ne',
  'lt-lf',
  'ki-xh',
  'qt-ij',
  'pu-ki',
  'of-ui',
  'bk-ej',
  'us-mb',
  'kt-xf',
  'uj-ux',
  'yg-ou',
  'on-ul',
  'ws-tb',
  'hm-df',
  'ta-vg',
  'wj-pn',
  'rk-ul',
  'cq-zn',
  'pz-qu',
  'oh-pn',
  'uy-gg',
  'jp-cn',
  'bz-py',
  'iu-po',
  'tg-xh',
  'wt-jz',
  'ho-lz',
  'oi-ug',
  'ch-zn',
  'mw-gr',
  'cv-kg',
  'vt-dz',
  'fg-ya',
  'sh-rv',
  'tu-al',
  'kv-ho',
  'ro-af',
  'py-li',
  'mi-bm',
  'ms-vd',
  'ox-mu',
  'nj-wl',
  'hx-eg',
  'fy-dz',
  'mo-mz',
  'us-ob',
  'pa-zy',
  'eh-js',
  'gj-ho',
  'rn-rf',
  'xu-nr',
  'ih-qt',
  'go-xz',
  'vx-zf',
  'qk-ez',
  'ej-df',
  'wq-nx',
  'zx-ir',
  'lq-ee',
  'jk-sh',
  'nf-wd',
  'yo-ti',
  'hz-su',
  'df-pn',
  'np-yk',
  'zn-ec',
  'zv-oc',
  'np-kd',
  'sr-gv',
  'tw-st',
  'wc-so',
  'en-ck',
  'vy-ne',
  'ja-fr',
  'dq-yk',
  'ij-iw',
  'se-cf',
  'cy-wu',
  'ey-im',
  'xt-xy',
  'ze-jj',
  'zi-zn',
  'iu-lf',
  'wi-rl',
  'zf-ad',
  'qq-ji',
  'kj-vp',
  'wz-hj',
  'cv-pe',
  'sy-hm',
  'ln-zj',
  'cx-gp',
  'ye-vs',
  'jp-wg',
  'pn-ej',
  'ys-dk',
  'iv-wy',
  'zc-gx',
  'lb-qw',
  'bl-eg',
  'yh-vm',
  'no-dp',
  'uq-av',
  'us-gx',
  'lk-gv',
  'gr-rz',
  'fh-ts',
  'wt-zu',
  'xs-el',
  'op-fb',
  'hp-wp',
  'xu-tk',
  'pe-km',
  'lb-kw',
  'ix-hd',
  'jz-uh',
  'im-dj',
  'sp-vo',
  'vp-rv',
  'xv-jl',
  'vi-op',
  'in-bv',
  'fn-hk',
  'md-dy',
  'in-sc',
  'mf-zh',
  'ju-lg',
  'ry-fh',
  'cn-fg',
  'sa-kb',
  'ot-pt',
  'ii-dv',
  'jp-wm',
  'ds-xv',
  'wq-mq',
  'gz-bm',
  'go-fy',
  'bg-we',
  'uz-hl',
  'mj-sx',
  'wg-iu',
  'zy-is',
  've-gq',
  'af-td',
  'jg-tr',
  'li-bz',
  'll-mv',
  'mo-fr',
  'wj-hm',
  'kr-ne',
  'uv-im',
  'dh-yy',
  'ce-pd',
  'na-jc',
  'hr-ei',
  'ou-pe',
  'cb-bd',
  'sq-il',
  'eg-dv',
  'yi-nz',
  'ms-zu',
  'vp-jx',
  'nk-nw',
  'vi-ek',
  'aa-nd',
  'nu-zr',
  'fm-if',
  'em-ti',
  'cl-xz',
  'ea-sq',
  'um-cj',
  'bm-dq',
  'kr-cn',
  'wo-uc',
  'fe-cj',
  'cj-rn',
  'ga-fz',
  'ge-oh',
  'ne-ya',
  'nx-ve',
  'hn-iy',
  'bq-aj',
  'os-en',
  'wg-gn',
  'jt-qw',
  'wz-lo',
  'tp-ww',
  'po-dm',
  'jt-wl',
  'ob-ls',
  'gq-yy',
  'pk-pw',
  'wu-fc',
  'ww-zb',
  'on-ho',
  'np-zi',
  'st-fx',
  'dk-ph',
  'hx-hj',
  'zn-iv',
  'ee-kg',
  'ir-kz',
  'kt-ca',
  'ng-bf',
  'zx-lg',
  'gd-xe',
  'th-gz',
  'kp-vh',
  'mv-ty',
  'hj-lo',
  'kt-gb',
  'sa-jh',
  'or-cw',
  'vo-lc',
  'nc-wc',
  'ku-ip',
  'ld-wq',
  'dh-mq',
  'lk-sr',
  'le-ld',
  'rj-fh',
  'au-ky',
  'ul-xk',
  'ai-ug',
  'ln-fj',
  'mj-ix',
  'mb-xj',
  'on-xk',
  'ln-bu',
  'wj-df',
  'ty-lz',
  'xh-xt',
  'mh-mk',
  'pb-ro',
  'jx-mh',
  're-hp',
  'xz-hh',
  'oe-xr',
  'un-pc',
  'ls-lz',
  'le-gq',
  'uy-fq',
  'nk-fg',
  'wg-dm',
  'kd-lx',
  'ox-jn',
  'yj-vm',
  'jx-cb',
  'by-wq',
  'zn-vb',
  'yn-zh',
  'ip-hp',
  'pm-xu',
  'uk-ge',
  'sh-hh',
  'xs-nl',
  'ti-ds',
  'cu-bc',
  'wl-lb',
  'sz-vp',
  'xf-gb',
  'wv-ig',
  'of-hm',
  'ld-mq',
  'se-aj',
  'bv-mx',
  'xj-ob',
  'bs-zb',
  'hz-qs',
  'qh-jl',
  'bi-vv',
  'ov-xs',
  'mf-hi',
  'zj-nu',
  'pq-lb',
  'ah-mg',
  'ix-yu',
  'pc-ys',
  'fv-ss',
  'dh-me',
  'gw-qj',
  'nt-hf',
  'jl-he',
  'jk-jx',
  'wx-ww',
  'yj-uc',
  'so-us',
  'ka-mh',
  'by-qn',
  'vm-wb',
  'sb-cg',
  'xe-ah',
  'hz-gp',
  'xl-ou',
  'gu-uv',
  'ie-by',
  'lm-rr',
  'mz-ji',
  'cf-li',
  'ju-fw',
  'er-lh',
  'cb-rv',
  'fw-fm',
  'rx-er',
  'bk-sy',
  'ec-yk',
  'jp-dm',
  'lf-dm',
  'cg-xq',
  'an-sy',
  'ur-tr',
  'jg-en',
  'uf-te',
  'no-la',
  'wp-qk',
  'yg-mj',
  'iu-ju',
  'gs-fe',
  'vb-ch',
  'oq-au',
  'zj-fj',
  'tc-rz',
  'do-fi',
  'cp-er',
  'ur-rp',
  'cw-ry',
  'ap-ar',
  'zh-xs',
  'ow-lb',
  'rc-xw',
  'fz-dz',
  'cq-zi',
  'ts-uf',
  'qy-ng',
  'pz-rl',
  'pf-wl',
  'tw-hn',
  'rd-qy',
  'ge-df',
  'zx-up',
  'wy-ec',
  'vh-wn',
  'jz-gm',
  'ib-gx',
  'wc-ac',
  'oc-zi',
  'ly-ez',
  'mp-sn',
  'bv-bf',
  'mu-av',
  'mj-cv',
  'er-jc',
  'jp-dt',
  'oe-ji',
  'wi-pz',
  'ky-mx',
  'wo-vp',
  'if-kw',
  'li-yv',
  'xq-lx',
  'ns-ai',
  'bb-fb',
  'kr-oa',
  'xz-ux',
  'jc-op',
  'fk-aw',
  'yd-ws',
  'so-dt',
  'ng-rr',
  'ap-fq',
  'wb-uc',
  'dv-wz',
  'kv-gj',
  'ev-uf',
  'yi-nk',
  'cs-ng',
  'di-po',
  'sq-ka',
  'nc-gd',
  'yk-si',
  'mx-rd',
  'vs-fq',
  'gh-pw',
  'tb-vy',
  'fg-ne',
  'tq-gw',
  'gn-wm',
  'bg-rc',
  'mx-bf',
  'tg-iy',
  'yj-sl',
  'qw-zo',
  'is-xn',
  'hz-cx',
  'xm-ga',
  'ph-ws',
  'pm-ea',
  'pj-kc',
  'pc-nl',
  'kq-tj',
  'zm-ah',
  'so-xj',
  'as-no',
  'zf-cu',
  'ok-ze',
  'sn-yc',
  'yb-hr',
  'fe-yb',
  'pn-sy',
  'pt-yd',
  'yk-zi',
  'dt-zv',
  'ca-cp',
  'tb-fp',
  'll-fb',
  'oq-vg',
  'qn-ad',
  'lt-xd',
  'um-ef',
  'kr-tb',
  'sz-mh',
  'or-ul',
  'xn-pa',
  'qz-yj',
  'jh-qw',
  'mq-ve',
  'dq-pa',
  'du-nt',
  'iy-pu',
  'ey-hz',
  'ld-nx',
  'xy-gc',
  'jz-td',
  'zp-kq',
  'yh-tc',
  'aj-yv',
  'oy-ac',
  'ub-uv',
  'ns-rj',
  'sv-ts',
  'jg-mz',
  'ku-dk',
  'sy-lr',
  'af-qc',
  'zu-vd',
  'kk-vs',
  'tx-jx',
  'pe-lq',
  'kx-dd',
  'tp-nh',
  'vx-cy',
  'on-cw',
  'wf-li',
  'nk-xl',
  'oo-ve',
  'ug-nt',
  'iz-zv',
  'eg-eq',
  'bd-gz',
  'um-bp',
  'th-bm',
  'ra-kv',
  'dm-di',
  'rr-in',
  'or-rk',
  'sx-cv',
  'fn-cw',
  'py-mp',
  'ds-ym',
  'xe-nc',
  'bu-fc',
  'oa-ya',
  'mf-ix',
  'ov-nl',
  'hn-pu',
  'ym-jl',
  'zf-bc',
  'ad-bc',
  'ls-iw',
  'yy-mq',
  'px-qn',
  'xq-io',
  'fn-le',
  'ba-ux',
  'mb-gl',
  'aa-mw',
  'tx-xr',
  'ue-ll',
  'yo-fi',
  'rf-ss',
  'iu-jp',
  'th-dq',
  'sd-do',
  'ib-us',
  'ef-bp',
  'mj-ky',
  'uk-wj',
  'mm-gs',
  'js-kd',
  'te-yd',
  'mw-qt',
  'ki-lw',
  'gv-ip',
  'fb-ga',
  'af-vd',
  'de-kz',
  'qb-wu',
  'no-eh',
  'kt-uz',
  'ak-et',
  'em-ul',
  'kd-cg',
  'ye-ua',
  'aw-lk',
  'qc-uh',
  'ux-iq',
  'ef-ss',
  'qo-yd',
  'fy-iq',
  'ce-mx',
  'om-qi',
  'mb-is',
  'sr-fk',
  'we-xk',
  've-ld',
  'he-xv',
  'io-lx',
  'ip-hi',
  'om-jg',
  'pt-ws',
  'pa-jd',
  'cw-xw',
  'ca-ze',
  'cy-bc',
  'ce-ng',
  'me-ao',
  'iv-yk',
  'zh-yu',
  'qm-ds',
  'ef-cs',
  'cm-oi',
  'xk-rp',
  'gv-hp',
  'jc-mv',
  'fg-yd',
  'cv-ky',
  'iz-so',
  'bq-bz',
  'jn-wg',
  'ok-jj',
  'hj-dv',
  'mm-fo',
  'qn-kp',
  'nc-nf',
  'gj-cd',
  'bz-qc',
  'in-ce',
  'rj-yr',
  'dp-xq',
  'ng-sa',
  'sq-ig',
  'bk-ui',
  'rv-jk',
  'le-oo',
  'wm-jn',
  'gw-vq',
  'kk-ar',
  'tb-ne',
  'md-qy',
  'if-xl',
  'cn-ot',
  'mm-fv',
  'df-oh',
  'll-xm',
  'ju-vv',
  'xv-qm',
  'ic-uj',
  'fx-xy',
  'ai-cm',
  'yn-pd',
  'as-io',
  'xe-wc',
  'kp-px',
  'su-dj',
  'fw-ma',
  'vg-aa',
  'oe-gi',
  'ij-ra',
  'el-ph',
  'fo-gs',
  'cl-mo',
  'dj-qs',
  'pe-mj',
  'pq-jt',
  'pd-pz',
  'sf-gi',
  'mk-rv',
  'ac-nf',
  'ux-go',
  'yb-kk',
  'sd-qh',
  'js-sb',
  'wo-ek',
  'up-bi',
  'jw-nz',
  'is-dl',
  'fo-uq',
  'ck-ja',
  'px-wn',
  'kq-ww',
  'os-mz',
  'rn-fv',
  'rp-mz',
  'iw-cd',
  'ij-zt',
  'fq-ar',
  'yg-sx',
  'rg-xu',
  'wc-nf',
  'kq-uq',
  'xz-iq',
  'pk-gz',
  'ul-we',
  'gs-rn',
  'no-lx',
  'ig-ea',
  'lo-eg',
  'kb-jh',
  'na-rx',
  'uq-tp',
  'du-an',
  'nv-uy',
  'nc-dd',
  'ot-ne',
  'qv-sp',
  'vs-yb',
  'ja-os',
  'dr-vp',
  'mb-iz',
  'av-wx',
  'sf-ok',
  'gq-ao',
  'hi-re',
  'ox-gn',
  'yo-jl',
  'np-cq',
  'qq-ca',
  'nd-ne',
  'om-ur',
  'yr-ug',
  'hw-hf',
  'dl-mi',
  'vb-ur',
  'hw-kj',
  'qt-nd',
  'ih-in',
  'rn-mm',
  'ai-nt',
  'tt-km',
  'nu-qb',
  'fz-iq',
  'uz-zr',
  'px-ri',
  'uz-zj',
  'ro-qe',
  'pt-vy',
  'cm-rj',
  'sz-jk',
  'sh-xr',
  'xj-iz',
  'ry-te',
  'fp-fa',
  'rx-bb',
  'mm-um',
  'xd-hd',
  'qm-sd',
  'pf-jh',
  'fq-qy',
  'lo-bm',
  'ee-ky',
  'dz-uj',
  'li-aj',
  'gv-og',
  'jn-iu',
  'xw-fn',
  'kt-ji',
  'mo-om',
  'cg-dz',
  'hw-ug',
  'lt-gn',
  'ty-fu',
  'un-et',
  'mu-pc',
  'fo-ef',
  'dq-is',
  'hh-ic',
  'fn-rc',
  'lf-xd',
  'xl-qk',
  'or-rc',
  'dy-ng',
  'hs-jt',
  'vb-yk',
  'ic-cl',
  'cp-mi',
  'yq-nt',
  'cv-km',
  'ss-gs',
  'yj-wo',
  'oz-yu',
  'rd-bf',
  'yy-ao',
  'pe-ee',
  'of-py',
  'tc-ps',
  'gx-iz',
  'zr-aq',
  'ph-un',
  'zy-xn',
  'dc-uq',
  'zi-xc',
  'vy-gg',
  'dd-gd',
  'us-iz',
  'ta-qu',
  'md-rr',
  'yh-rz',
  'vv-kz',
  'oq-qt',
  'vm-qz',
  'yc-ia',
  'ya-vy',
  'wv-dw',
  'fc-nh',
  'mx-sc',
  'vm-sl',
  'hi-pj',
  'tr-rp',
  'ca-xf',
  'sb-xq',
  'hf-an',
  'dr-jk',
  'zf-wn',
  'sr-jy',
  'tx-mh',
  'gd-wd',
  'uq-pw',
  'qj-ii',
  'bb-lh',
  'pd-mf',
  'fl-ho',
  'tj-ix',
  'el-ak',
  'bi-dx',
  'ly-yi',
  'hl-nh',
  'rl-aa',
  'us-xj',
  'qi-tr',
  'zh-kf',
  'me-nx',
  'fq-ei',
  'ki-gc',
  'vd-wt',
  'jf-xj',
  'bv-sc',
  'ek-yh',
  'lo-qj',
  'aj-sn',
  'cj-rf',
  'ad-px',
  'fa-cx',
  'qu-oq',
  'sq-rg',
  'xd-iu',
  'xu-wv',
  'io-cg',
  'nx-dh',
  'kw-wp',
  'fk-qw',
  'hl-zr',
  'rf-fv',
  'wc-gd',
  'jc-xm',
  'dp-io',
  'yo-qh',
  'pb-td',
  'su-fp',
  'qc-gm',
  'ij-gj',
  'ma-vv',
  'vp-tx',
  'om-fr',
  'ga-op',
  'nx-mp',
  'sb-io',
  'yv-se',
  'tp-bs',
  'oo-mq',
  'hm-oh',
  'pn-bk',
  'il-ka',
  'mj-au',
  'pk-dq',
  'pt-fg',
  'sn-py',
  'dr-mk',
  'ji-xf',
  'ad-vx',
  'rk-we',
  'uv-su',
  'qi-ja',
  'nj-jh',
  'om-os',
  'if-nw',
  'db-up',
  'qq-xf',
  'wm-xd',
  'su-ey',
  'di-gn',
  'cg-as',
  'nl-ow',
  'lb-jh',
  'yn-tj',
  'fy-ba',
  'zm-ac',
  'le-wq',
  'ea-dw',
  'up-fw',
  'fr-qi',
  'hr-ua',
  'po-wg',
  'um-fo',
  'fm-nk',
  'ap-gg',
  'hd-dx',
  'ip-aw',
  'ie-uh',
  'qb-uz',
  'hx-yq',
  'cw-hk',
  'sa-ow',
  'zy-mi',
  'py-yc',
  'ot-kr',
  'jl-qm',
  'ps-qz',
  'ba-go',
  'ri-kp',
  'io-rm',
  'bi-fw',
  'um-gs',
  'so-gx',
  'hs-pf',
  'fi-em',
  'wm-wg',
  'yq-rj',
  'sl-rz',
  'mu-uq',
  'oy-ah',
  'il-tc',
  'rg-tl',
  'ai-yr',
  'hk-kx',
  'fa-ub',
  'ih-oq',
  'hh-ba',
  'ya-tb',
  'iy-xt',
  'dw-hj',
  'yo-ym',
  'fx-iy',
  'rc-rm',
  'hd-tf',
  'zm-gd',
  'xz-fc',
  'ul-cw',
  'ey-gu',
  'nj-pq',
  'gz-pa',
  'zo-sa',
  'xc-si',
  'oi-yq',
  'iy-ki',
  'cv-au',
  'pb-vd',
  'ph-xs',
  'qo-pt',
  'in-dy',
  'zr-gx',
  'wc-ah',
  'wc-dd',
  'gs-rf',
  'cn-ne',
  'ov-xp',
  'nu-ln',
  'qk-nw',
  'lt-ox',
  'vi-ps',
  'nr-il',
  'yu-tf',
  'gh-zp',
  'bp-re',
  'vp-mh',
  'hk-px',
  'dz-ux',
  'vq-lo',
  'kt-jj',
  'fc-zr',
  'bd-dl',
  'hj-gw',
  'ok-cp',
  'lr-df',
  'fb-jc',
  'uv-ey',
  'cx-qs',
  'kp-vx',
  'yc-cf',
  'eq-tq',
  'tx-mk',
  'js-as',
  'pq-kb',
  'aq-nh',
  'oz-mf',
  'oe-ze',
  'we-cw',
  'nk-kw',
  'ho-ty',
  'ow-jh',
  'cy-ad',
  'cs-cj',
  'hi-gv',
  'gl-fy',
  'uf-sv',
  'qn-vh',
  'yo-sd',
  'pm-dw',
  'ri-cy',
  'pm-tl',
  'qb-nh',
  'qi-os',
  'ja-en',
  'uj-fz',
  'sh-kj',
  'xk-xw',
  'jt-nj',
  'rv-ue',
  'xu-ea',
  'pt-lg',
  'fb-xm',
  'fr-ur',
  'pc-dk',
  'vm-uc',
  'nu-uz',
  'ma-db',
  're-kc',
  'cy-by',
  'dw-tl',
  'jy-gv',
  'ki-tw',
  'ta-oq',
  'rc-xk',
  'ux-cl',
  'gh-mu',
  'sc-lm',
  'ip-kc',
  'eg-tq',
  'md-ma',
  'll-jc',
  'ki-tg',
  'lm-ng',
  'ys-xs',
  'hw-ns',
  'ij-ty',
  'yq-ns',
  'qm-tk',
  'ow-qw',
  'zi-wy',
  'kk-ap',
  'yg-ky',
  'dx-tf',
  'pk-mi',
  'iv-cq',
  'pd-yu',
  'fh-vo',
  'po-lf',
  'rv-sz',
  'rv-mh',
  'mz-fr',
  'np-wy',
  'sf-jj',
  'yi-ez',
  'gc-tw',
  'of-uk',
  'jj-xf',
  'ob-zc',
  'fw-vv',
  'mp-ia',
  'vb-cq',
  'fp-hz',
  'qi-nc',
  'cf-aj',
  'ld-yy',
  'em-qh',
  'vx-wn',
  'aa-wi',
  'go-hh',
  'om-rp',
  'er-ll',
  'qq-ze',
  'ph-ov',
  'dc-av',
  'av-zb',
  'dl-xt',
  'gc-st',
  'oi-ns',
  'fg-oa',
  'hk-or',
  'wi-mw',
  'pf-sa',
  'ge-sy',
  'jf-sf',
  'wp-xl',
  'iy-st',
  'rk-xk',
  'ov-dk',
  'og-fk',
  'oe-jj',
  'kz-ma',
  'yb-ye',
  'zu-ie',
  'eq-ii',
  'qo-oa',
  'im-su',
  'fo-rn',
  'zc-so',
  'ig-sf',
  'jn-lt',
  'ln-wu',
  'ki-xy',
  'aj-yc',
  'wv-sq',
  'db-de',
  'hw-ai',
  'tp-pw',
  'ar-ei',
  'ur-mo',
  'li-ea',
  'lt-rd',
  'xd-wg',
  'ey-fp',
  'ck-tr',
  'up-lg',
  've-ao',
  'jl-fi',
  'eh-as',
  'rd-lm',
  'vv-zx',
  'by-kp',
  'ro-ms',
  'kg-ou',
  'uz-nh',
  'ox-jp',
  'le-ve',
  'vg-hs',
  'rv-dr',
  'kc-hp',
  'hx-qj',
  'ry-ev',
  'dv-tq',
  'uh-vd',
  'cn-ws',
  'ac-mg',
  'ir-up',
  'de-fw',
  'db-al',
  'on-bg',
  'tt-pe',
  'tj-pd',
  'dm-ox',
  'wg-lt',
  'nt-oi',
  'bz-yc',
  'xm-rx',
  'zt-zi',
  'ck-ur',
  'mb-gx',
  'uj-ba',
  'wo-wb',
  'dv-qj',
  'yx-yt',
  'cm-an',
  'sn-bq',
  'hs-zo',
  'nz-qk',
  'ib-gl',
  'iv-ec',
  'up-al',
  'jn-gn',
  'bg-fn',
  'nw-ly',
  'ka-pm',
  'hs-qw',
  'dj-hz',
  'yk-oc',
  'cx-fp',
  'tt-ee',
  'ez-bf',
  'aw-pj',
  'cp-ji',
  'uv-do',
  'ux-hh',
  'qe-le',
  'hi-aw',
  'px-by',
  'qt-qu',
  'hs-wl',
  'di-jp',
  'og-lk',
  'yu-yn',
  'im-hz',
  'em-yo',
  'fu-lz',
  'de-al',
  'xu-tl',
  'ls-kv',
  'qv-yx',
  'ds-tx',
  'ns-fp',
  'bk-wj',
  'al-fw',
  'yx-vo',
  'kc-gv',
  'sc-md',
  'ty-gj',
  'wm-di',
  'ox-xd',
  'ns-du',
  'qi-mo',
  'kz-zx',
  'gg-fq',
  'hr-fq',
  'll-op',
  'cd-fu',
  'dx-yn',
  'ts-yt',
  'ys-ak',
  'xj-ib',
  'db-xf',
  'bf-ce',
  'bl-ii',
  'qm-he',
  'aa-qu',
  'os-iz',
  'fj-bu',
  'pb-ms',
  'ic-xz',
  'jg-mo',
  'zb-wi',
  'ie-ms',
  'yv-cf',
  'yu-tj',
  'dz-ba',
  'vv-up',
  'mm-bp',
  'zj-hl',
  'fu-iw',
  'wl-qw',
  'ic-fz',
  'tg-fx',
  'xe-wd',
  'zu-gm',
  'ln-nh',
  'ou-lq',
  'ye-ar',
  'tq-hx',
  'yb-ap',
  'bp-ss',
  're-og',
  'qb-pa',
  'kz-al',
  'lt-iu',
  'll-zn',
  'he-do',
  'oe-ca',
  'tn-yn',
  'fj-uz',
  'ev-te',
  'fl-gj',
  'fu-gj',
  'cb-sz',
  'if-wp',
  'ry-vo',
  'gu-fa',
  'ro-cm',
  'sq-pm',
  'dk-el',
  'ih-mw',
  'mv-fb',
  'bg-xk',
  'wy-yk',
  'ja-jg',
  'rm-lx',
  'xs-ku',
  'iy-gc',
  'xw-bg',
  'dq-jd',
  'ey-gp',
  'wy-xc',
  'ac-ah',
  'jx-rv',
  'md-mx',
  'xq-eh',
  'nj-sa',
  'vx-qn',
  'yo-he',
  'vy-oa',
  'di-ox',
  'er-op',
  'qv-vo',
  'jw-qk',
  'yr-hf',
  'oe-qq',
  'qn-zf',
  'yh-wo',
  'wu-fj',
  'ec-ch',
  'yj-rz',
  'dx-kf',
  'td-rg',
  'io-kd',
  'mz-en',
  'tw-xh',
  'sd-em',
  'cf-bq',
  'ky-pe',
  'yj-qq',
  'wf-eg',
  'zb-uq',
  'qm-qh',
  'iq-hh',
  'ku-ys',
  'bc-by',
  'ju-ir',
  'dp-sb',
  'hs-kb',
  'qw-sa',
  'mw-rl',
  'fp-im',
  'wz-ii',
  'ih-vg',
  'lz-fl',
  'rf-bp',
  'bv-md',
  'ee-km',
  'cv-ou',
  've-dh',
  'dw-il',
  'vs-ei',
  'yx-ij',
  'gn-wl',
  'jh-zo',
  'yq-cm',
  'dq-mi',
  'hp-jy',
  'xd-gn',
  'ip-jy',
  'au-ou',
  'dv-gw',
  'lk-hp',
  'hi-hp',
  'au-pe',
  'oi-yr',
  'ii-vq',
  'yg-pe',
  'gw-ii',
  'oa-pt',
  'hw-rj',
  'fx-gc',
  'kd-sb',
  'zt-iw',
  'rg-ea',
  'wy-uy',
  'ux-xu',
  'ze-ji',
  'bg-np',
  'qe-mq',
  'hf-oi',
  'zo-nj',
  'dw-ig',
  'wb-ek',
  'pb-jz',
  'wb-yj',
  'yv-bq',
  'kf-af',
  'pm-nr',
  'rf-fo',
  'kz-bi',
  'im-ck',
  'ya-cv',
  'zo-ow',
  'et-ov',
  'hn-st',
  'nl-ph',
  'fq-ye',
  'zb-mu',
  'nc-wd',
  'tu-zm',
  'tn-tf',
  'vg-pz',
  'bl-eq',
  'yr-yq',
  'vi-wb',
  'lo-eq',
  'fi-ds',
  'ls-zt',
  'zj-bu',
  'ar-gg',
  'iw-ba',
  'as-ra',
  'ua-vs',
  'vt-fz',
  'lr-oh',
  'jh-mm',
  'ac-wd',
  'bz-mp',
  'ip-pj',
  'fh-sv',
  'gl-so',
  'ld-dh',
  'ba-cl',
  'te-yt',
  'tn-hd',
  'hd-mf',
  'ps-bl',
  'mv-xm',
  'tt-cv',
  'vq-hj',
  'cx-su',
  'dt-us',
  'rm-no',
  'zv-mb',
  'mz-om',
  'hj-eg',
  'zj-aq',
  'rp-jg',
  'xk-cw',
  'en-om',
  'un-rl',
  'ya-pt',
  'ai-du',
  'tl-nr',
  'cx-ub',
  'fi-xv',
  'tr-os',
  'se-ia',
  'ce-bv',
  'fy-cl',
  'nt-an',
  'tf-yn',
  'ek-uc',
  'gm-ie',
  'ca-ji',
  'yv-wf',
  'qm-ti',
  'ue-mv',
  'gu-fp',
  'ld-ao',
  'td-vd',
  'wg-ua',
  'dp-rm',
  'if-yi',
  'gd-ah',
  'ip-og',
  'bs-kq',
  'sz-dr',
  'ta-gr',
  'on-kx',
  'kf-mf',
  'wo-rz',
  'zi-ch',
  'lg-kz',
  'nl-xp',
  'ar-aa',
  'cu-ri',
  'uk-oh',
  'wf-bq',
  'yu-mf',
  'dl-gz',
  'yx-sp',
  'lf-gn',
  'fh-uf',
  'sq-nr',
  'yc-li',
  'wb-hl',
  'nk-jw',
  'ap-nv',
  'nx-ao',
  'zb-wx',
  'oc-xc',
  'ug-du',
  'or-kx',
  'yt-uf',
  'wf-se',
  'wl-sa',
  'rg-dw',
  'jk-eq',
  'an-ai',
  'ye-kk',
  'dm-sx',
  'ba-xz',
  'yg-kg',
  'iv-zm',
  'xu-ig',
  'rp-en',
  'iq-ba',
  'yy-ve',
  'fc-qb',
  'du-yq',
  'tu-wc',
  'su-ub',
  'ry-yx',
  'fg-ot',
  'fy-uj',
  'uy-yb',
  'xf-ze',
  'ox-lf',
  'jt-pf',
  'xn-th',
  'll-rx',
  'ng-rd',
  'nx-le',
  'ec-vb',
  'mq-le',
  'fz-ux',
  'ro-gm',
  'uh-af',
  'jl-do',
  'nl-dk',
  'yt-qv',
  'mu-zp',
  'uh-mk',
  'vb-si',
  'tw-iy',
  'vs-nv',
  'mx-in',
  'wq-gq',
  'we-fn',
  'cu-px',
  'ow-wl',
  'ja-mz',
  'qb-zj',
  'tc-vi',
  'vq-wu',
  'pb-qc',
  'vg-gr',
  'xh-fx',
  'ue-os',
  'mp-aj',
  'mi-is',
  'fk-ip',
  'nv-fq',
  'ly-qk',
  'os-rp',
  'ap-ei',
  'ar-nv',
  'cu-wn',
  'yi-oz',
  'wy-ch',
  'ws-ne',
  'pj-og',
  'qj-go',
  'lh-bd',
  'aw-sr',
  'zb-gh',
  'oa-cn',
  'ps-wb',
  'bb-er',
  'qb-bu',
  'kv-fu',
  'iq-go',
  'qh-ti',
  'mh-cb',
  'uy-ei',
  'ti-xv',
  'cj-ef',
  'na-er',
  'xw-on',
  'rj-oi',
  'bq-mp',
  'dz-iq',
  'as-rm',
  'ta-fw',
  'eh-cg',
  'is-gz',
  'lm-in',
  'vm-wo',
  'iu-wm',
  'ej-wj',
  'qq-jj',
  'ts-ry',
  'bz-aj',
  'ww-mu',
  'bi-ir',
  'yb-ei',
  'ju-up',
  'pz-gr',
  'ua-fq',
  'zv-zc',
  'js-dp',
  'mf-yn',
  'rn-bp',
  'ls-fl',
  'op-ue',
  'td-zu',
  'rl-qu',
  'oy-ca',
  'lw-hn',
  'yx-sv',
  'mi-jd',
  'mp-li',
  'cf-qv',
  'rm-xq',
  'pk-jd',
  'mi-gz',
  'wv-ea',
  'ku-nl',
  'bp-fv',
  'rp-ja',
  'ix-dx',
  'yx-lc',
  'lw-tg',
  'aw-jy',
  'ih-gr',
  'px-zf',
  'eq-hj',
  'ku-we',
  'th-zy',
  'dy-lm',
  'hh-xc',
  'uq-gh',
  'pu-xt',
  'zc-xj',
  'xc-zn',
  'zn-si',
  'pk-bm',
  'xh-iy',
  'fx-lw',
  'fa-hn',
  'jf-dt',
  'gp-yt',
  'nw-nz',
  'rr-dy',
  'ue-lh',
  'cs-mm',
  'is-pk',
  'lz-iw',
  'mu-bs',
  'qj-hj',
  'bc-qn',
  'gh-wx',
  'yy-nx',
  'ya-qo',
  'jl-sd',
  'ns-ug',
  'qv-ts',
  'zn-wy',
  'jw-xl',
  'ev-om',
  'fw-kz',
  'or-on',
  'xs-ak',
  'ce-dy',
  'no-js',
  'by-ri',
  'nf-tu',
  'qh-fi',
  'qj-vq',
  'om-ck',
  'dq-zy',
  'dr-cb',
  'pa-th',
  'ma-ju',
  'hx-bl',
  'wt-qc',
  'un-xp',
  'ek-sl',
  'po-xd',
  'hl-fc',
  'iy-xy',
  'gw-eq',
  'of-ej',
  're-ip',
  'ma-al',
  'wt-af',
  'pa-pk',
  'fj-sq',
  'lc-ev',
  'og-jy',
  'ot-yu',
  'ww-bs',
  'iz-dt',
  'df-uk',
  'ne-qo',
  'sh-dr',
  'rg-il',
  'wn-bc',
  'fq-yb',
  'xh-xy',
  'rd-bv',
  'pw-ww',
  'qk-nk',
  'mo-tr',
  'um-ss',
  'pm-xj',
  'de-vv',
  'ye-ei',
  'ra-fu',
  'rc-rk',
  'kv-ij',
  'rc-cw',
  're-sr',
  'si-np',
  'uk-bk',
  'pq-sa',
  'uq-ww',
  'iz-jf',
  'wu-nh',
  'uv-cx',
  'gb-pu',
  'if-qk',
  'fp-gp',
  'we-on',
  'cu-by',
  'on-rk',
  'ph-ak',
  'uh-gm',
  'ty-ls',
  'nt-rj',
  'ez-fm',
  'oe-ok',
  'sl-vi',
  'kf-oz',
  'lw-xy',
  'al-ir',
  'ds-yo',
  'jw-ly',
  'wt-xv',
  'fk-jy',
  'dd-uf',
  'iu-di',
  'vp-sh',
  'ce-rd',
  'ei-ua',
  'fj-nu',
  'dh-gq',
  'wf-mp',
  'in-ng',
  'pe-kg',
  'se-yc',
  'nw-yi',
  'qe-nx',
  'de-up',
  'uv-dj',
  'qq-gb',
  'cs-gs',
  'fq-kk',
  'kb-zo',
  'mx-qy',
  'sn-yn',
  'jx-sh',
  'vb-xc',
  'tk-pm',
  'tn-mf',
  'wm-dm',
  'fo-ss',
  'uy-ua',
  'oo-wq',
  'em-do',
  'pu-xh',
  'fr-jg',
  'bm-is',
  'ej-cx',
  'lf-wg',
  'ua-yb',
  'cg-la',
  'xs-xp',
  'yj-yh',
  'fg-vy',
  'ys-xp',
  'zy-pk',
  'qv-ry',
  'hw-an',
  'di-nd',
  'tt-sx',
  'gc-he',
  'si-iv',
  'im-ub',
  'fl-cd',
  'gl-us',
  'kk-ei',
  'mp-fi',
  'fh-yt',
  'tl-ea',
  'fn-on',
  'ba-vt',
  'lq-yg',
  'bv-lm',
  'yg-ee',
  'tj-tn',
  'ji-jj',
  'ow-kb',
  'wd-mg',
  'hi-fk',
  'ey-zf',
  'fb-us',
  'hf-ug',
  'kb-pf',
  'lr-hm',
  'lz-kv',
  'vi-vm',
  'gq-hz',
  'tx-jk',
  'ig-pm',
  'sf-ji',
  'gg-ua',
  'jk-xr',
  'ia-sn',
  'bs-ee',
  'cd-ty',
  'ai-hf',
  'ah-nf',
  'ez-if',
  'fm-kw',
  'bb-na',
  'zp-tp',
  'qu-wi',
  'db-ir',
  'bc-px',
  'by-vh',
  'hr-uy',
  'dd-nf',
  'fi-ym',
  'dl-th',
  'un-ak',
  'xv-yo',
  'fb-lh',
  'tq-vq',
  'cu-kp',
  'td-qc',
  'fy-ic',
  'jd-xn',
  'wj-ui',
  'lr-bk',
  'yt-vo',
  'gh-sr',
  'xy-du',
  'ym-yr',
  'ta-rl',
  'se-bq',
  'fn-oh',
  'ui-ge',
  'gn-iu',
  'uy-ar',
  'nz-fm',
  'dz-cl',
  'qt-wi',
  'kg-sx',
  'le-me',
  'dr-jx',
  'eh-sb',
  'gj-ls',
  'py-bq',
  'qy-rr',
  'kc-jy',
  'hn-xh',
  'db-ju',
  'su-qs',
  'jw-ez',
  'jj-gi',
  'zx-de',
  'tx-rv',
  'zo-jt',
  'au-tt',
  'kk-uy',
  'mp-se',
  'fo-cj',
  'ah-dd',
  'lc-ry',
  'jz-vd',
  'fh-lc',
  'la-as',
  'an-rj',
  'gp-ub',
  'ob-ib',
  'wv-pm',
  'oe-sf',
  'dd-wd',
  'bl-lo',
  'cf-sn',
  'kr-yd',
  'ih-ta',
  'sb-dr',
  'ze-gb',
  'rx-ue',
  'ku-pc',
  'hn-xt',
  'gs-tn',
  'kd-dp',
  'zh-tn',
  'rj-hf',
  'op-lh',
  'me-wq',
  'fm-gr',
  'cp-kt',
  'cs-fv',
  'kc-og',
  'ho-fu',
  'ug-an',
  'ca-sf',
  'yx-ev',
  'mg-tu',
  'lc-qv',
  'av-zp',
  'pk-th',
  'om-tr',
  'km-ou',
  'wz-eg',
  'wt-ie',
  'kz-up',
  'aj-wf',
  'zo-pf',
  'cm-ug',
  'sl-qz',
  'mm-rf',
  'qi-ur',
  'bb-rm',
  'qw-pq',
  'wi-gr',
  'la-io',
  'dx-mf',
  'lf-jp',
  'gi-fz',
  'dv-lo',
  'ki-xt',
  'cs-fo',
  'tp-kq',
  'vd-ro',
  'bd-mi',
  'gq-mq',
  'lc-pb',
  'wu-uz',
  'zb-zp',
  'mv-na',
  'av-bs',
  'lq-sx',
  'vp-cb',
  'td-ie',
  'gh-dc',
  'xm-op',
  'yr-du',
  'hp-pj',
  'ca-gi',
  'fh-te',
  'qi-ck',
  'ot-tb',
  'wi-nd',
  'gm-wt',
  'ow-nj',
  'yt-sp',
  'kk-ir',
  'ro-jz',
  'ev-ts',
  'or-xk',
  'cn-yd',
  'pa-bm',
  'wo-vi',
  'vs-tf',
  'jc-lh',
  'vg-mw',
  'wp-jw',
  'wj-oh',
  'ar-hr',
  'fr-en',
  'ie-af',
  'bb-xm',
  'nv-gg',
  'ly-nz',
  'py-cf',
  'cu-ad',
  'po-wm',
  'bd-is',
  'im-gu',
  'gh-av',
  'fy-xz',
  'dy-bf',
  'qz-xh',
  'hk-xk',
  'tf-pd',
  'zp-hr',
  'nr-tk',
  'av-tp',
  'fn-rk',
  'cd-zt',
  'jl-ri',
  'ia-li',
  'zn-oc',
  'pu-st',
  'kx-rk',
  'ga-rx',
  'xt-gc',
  'ad-wn',
  'st-xy',
  'rm-js',
  'hw-nt',
  'qv-sv',
  'et-xs',
  'wo-tc',
  'ch-np',
  'sz-jx',
  'zc-gl',
  'zo-pq',
  'um-fv',
  'fh-qv',
  'zx-fw',
  'hd-zh',
  'av-pw',
  'ak-pc',
  'fi-he',
  'mk-sz',
  'ij-cd',
  'di-jn',
  'nv-jt',
  'fb-er',
  'lc-te',
  'wq-yy',
  'ld-gq',
  'ps-sl',
  'vg-rl',
  'xn-dq',
  'dk-et',
  'ju-al',
  'yr-ns',
  'ld-oo',
  'cx-im',
  'kv-ty',
  'cq-ch',
  'la-rm',
  'fb-ue',
  'hn-xy',
  'mg-gq',
  'la-lx',
  'ms-td',
  'fy-vt',
  'aa-pz',
  'ip-lk',
  'wz-vq',
  'st-lw',
  'lr-ej',
  'pn-of',
  'xl-nw',
  'wb-nt',
  'cj-mm',
  'cy-qn',
  'jf-gx',
  'sl-yh',
  'bb-jc',
  'du-oi',
  'lz-gj',
  'fc-zj',
  'sz-xr',
  'mk-jk',
  'ey-cx',
  'bv-st',
  'ub-hz',
  'sz-tg',
  'lt-po',
  'bl-tq',
  'mi-xn',
  'fk-lk',
  'as-dp',
  'zo-up',
  'xc-cq',
  'ry-sp',
  'uy-ye',
  'gu-dj',
  'zt-lz',
  'kp-yc',
  'la-xq',
  'kg-km',
  'yu-hd',
  'jn-et',
  'fp-dj',
  'ta-aa',
  'tl-md',
  'lg-ir',
  'ya-ot',
  'iu-ox',
  'wq-ve',
  'ee-au',
  'sc-ng',
  'vm-rz',
  'yn-hd',
  'dr-kj',
  'ib-mb',
  'qn-ri',
  'wt-pb',
  'lm-md',
  'dv-ao',
  'sr-kc',
  'zu-qc',
  'ds-jl',
  'km-au',
  'xf-gi',
  'pj-sr',
  'sy-ui',
  'ur-mz',
  'pw-mu',
  'bd-dq',
  'bz-cf',
  'ok-ji',
].map((str) => str.split('-'));

function uniqSortedFiltered(arr) {
  return uniq(
    arr
      .filter(pair => pair.filter(p => /^t[a-z]$/.test(p)).length > 0)
      .map(pair => pair.sort().join('-'))
  )
    .map(pair => pair.split('-'))
    .sort((a, b) => a[0].localeCompare(b[0]));
}

function isPair(input, a, b) {
  return input.filter(pair => pair.includes(a) && pair.includes(b)).length > 0;
}

function part1(input) {
  let combinedConnections = [];
  let usedPairs = [];

  for (let i = 0; i < input.length; i++) {
    const [a, b] = input[i];
    // cg-aq

    for (let j = 0; j < input.length; j++) {
      if (i != j) {
        const [c, d] = input[j];
        // cg-yn

        if (((a == c && isPair(input, b, d)) || (a == d && isPair(input, b, c)) || (b == c && isPair(input, a, d)) || (b == d && isPair(input, a, c))) && ![a, b].includes(d)) {
          const trio = [a, b, d];

          if (trio.filter(p => /^t[a-z]$/.test(p)).length > 0) {
            combinedConnections.push([a, b, d]);
            usedPairs.push(input[i]);
            usedPairs.push(input[j]);
          }

        }
      }
    }
  };

  return [uniqSortedFiltered(combinedConnections), usedPairs];
}

function findPair(input, reqPair) {
  const pairs = [];
  for (let pair of input) {
    if (pair.includes(reqPair[0]) || pair.includes(reqPair[1])) {
      pairs.push(pair);
    }
  }
  return pairs;
}

function groupPairs(input) {


}

function part2(input) {
  const graph = new Map();

  for (const [a, b] of input) {
    if (!graph.has(a)) graph.set(a, new Set());
    if (!graph.has(b)) graph.set(b, new Set());
    graph.get(a).add(b);
    graph.get(b).add(a);
  }

  const P = Array.from(graph.keys());
  const R = [];
  const X = [];
  const cliques = [];

  bronKerbosch(R, P, X, graph, cliques);

  return cliques.sort((a, b) => a.length > b.length ? -1 : 1).at(0).sort().join(',');
}

console.log(part2(final_input));